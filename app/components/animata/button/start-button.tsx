...
export default function StartButton({ text = 'Get started', className }: IGetStartedButtonProps) {
	return (
		<div className='min-h-10 w-48'>
			<button
				className={cn(
					'group flex h-10 w-40 items-center justify-center gap-3 rounded-lg bg-transparent p-2 font-bold transition-colors duration-100 ease-in-out w-32 bg-gradient-to-r from-blue-400 to-sky-300 hover:bg-orange-600',
					className
				)}
			>
				<span
					className={cn(
						'text-neutral-100 transition-colors duration-100 ease-in-out group-hover:text-amber-100'
					)}
				>
					{text}
				</span>
