...
function RevealImageListItem({ text, images }: ShowImageListItemProps) {
	const container = 'absolute right-8 -top-1 z-40 h-36 w-24'
	const effect =
	...
	return (
		<div className='group relative h-fit w-fit overflow-visible py-8'>
			<p className='text-4xl font-black text-foreground transition-all duration-500 group-hover:opacity-40'>
				{text}
			</p>
			...
			<div
				className={cn(
					container,
					'translate-x-0 translate-y-0 rotate-0 transition-all delay-150 duration-500 group-hover:translate-x-12 group-hover:translate-y-3 group-hover:rotate-12'
				)}
			>
...
export default function RevealImageList() {
	const items: ShowImageListItemProps[] = [
		{
			text: 'Services, Payments, Subscriptions, ',
			images: [
				{
